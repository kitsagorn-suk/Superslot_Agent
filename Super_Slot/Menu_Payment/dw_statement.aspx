<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="dw_statement.aspx.cs" Inherits="Pretty_Gaming.Menu_Payment.dw_statement" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    ...
    <script type="text/javascript">
        ...
        function Search_Click() {
            NumPage = 0;
            NumPage2 = 0;
            var username = $("#searchLoginname").val();
            GetDataAll("", username);
        }

        function GetDataAll(ID, username) {
            GetData(ID, username);
        }
        ...
        const postData = async (num, ID, username) => {
            if (num == 0) {
                num = num + 1;
            }
            ...
            const requestAwait = await fetchDataSite(`${apiURL}/v1/report/transaction_statement`, 'POST', "include", parameter)
                    ...
                    if (response.total != 0) {
                        for (var i = 0; i < response.data.detail.length; i++) {
                            ...
                            if (Position == "Member") {
                                ...
                                htmlData += "<td><a onclick='GetDataAll(`" + obj.userID + "`)' class='linkNav overflow ellipsis' title='" + obj.username + "'>" + obj.username + "</a></td>";
                            }
                        ...
                        var deposit = parseFloat(response.data.summary.deposit).toFixed(2);
                        var withdraw = parseFloat(response.data.summary.withdraw).toFixed(2);
                        var balance = parseFloat(response.data.summary.balance).toFixed(2);
                        ...
                        for (var i = 0; i < response.data.parentList.length; i++) {
                            ...
                            if (i == 0) {
                                NavMenu += "<a class='linkNav' onclick='GetDataAll(`" + obj.id + "`)'>" + obj.username + "</a>";
                            } else {
                                NavMenu += " / <a class='linkNav' onclick='GetDataAll(`" + obj.id + "`)'>" + obj.username + "</a>";
                            }
                        }
        ...
        const postGetTable = async (num, ID) => {
            $("#tbData > tbody , #navMenu").html("");
            const parameter = {
        ...
        const postGetTableDetail = async (num, ID) => {
            const parameter = {
                startDate: $("#startdate").val(),
                startTime: $("#starttime").val() + ":00",
                endDate: $("#todate").val(),
