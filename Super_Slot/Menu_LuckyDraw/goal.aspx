<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="goal.aspx.cs" Inherits="Pretty_Gaming.Menu_LuckyDraw.goal" %>
        ...
        <table class="table table-border" id="tbData">
            <thead class="rgba-green-slight">
                <tr>
                    ...
                    <th style='width: 5%; text-align: right; padding-right: 5px;' set-lan='text:Deposit Amount'></th>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body mx-3">
                    ...
                    <div class="form-group row">
                        <label for="username" class="col-4 col-form-label" style="text-align: right;" set-lan="html:Deposit Amount-"></label>
                        <div class="col-8">
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    <script>
        ...
        function btnSave(status) {
            ...
            if (Goal == "" || Goal == null) {
                document.getElementById("lbAlert").setAttribute("set-lan", "text:missing 'Deposit Amount' field");
                SetLan(localStorage.getItem("Language"));
        ...
        function btnEdit(goal, turnover) {
        ...
        const getData = async () => {
            const requestAwait = await fetchDataSite(`${apiURL}/v1/lotto/get/config`, 'GET', "include", {})
            ...
            if (response.messageCode == 0000 || response.messageCode == null) {
                ...
                if (response.data != null) {
                    ...
                    var depositAmount = response.data.depositAmount.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");

                    htmlData += "<tr>";
                    htmlData += "<td style='width: 1%;' class='aligncenter'>" + no + "</td>";
                    htmlData += "<td class='alignright'>" + depositAmount + "</td>";
                    htmlData += "<td class='alignright'>" + turnover + "</td>";
                    htmlData += "<td style='width: 1%;' class='aligncenter colEdit zonenone'><a class='link' onclick='btnEdit(`" + depositAmount + "`,`" + turnover + "`)'><i class='fas fa-pencil-alt'></i></a></td>";
                    htmlData += "</tr>";
                    no++;
        ...
        const postEdit = async () => {
            var DepositAMT = $('#Goal').val();
            var Turnover = $('#Turnover').val();
            DepositAMT = DepositAMT.replaceAll(",", "");
            ...
            const requestAwait = await fetchDataSite(`${apiURL}/v1/lotto/set/config`, 'POST', "include", parameter)
            const response = await requestAwait.json()
