<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="tournament_report.aspx.cs" Inherits="Pretty_Gaming.Menu_Report.tournament_report" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active txtMem" set-lan="text:Tournament Report"></li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-md-8">
            <h1 set-lan="text:Tournament Report"></h1>
        </div>
    </div>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    ...
    <script type="text/javascript">      
        ...
        const postData = async (status, userID, username, num) => {
            ...
            var parameter = {};
            if (status == 1) {
                URL = `${apiURL}/v1/tournament/report`;
                parameter = {
                ...
                URL = `${apiURL}/v1/tournament/report/member`;
                parameter = {
                    startDate: $("#startdate").val(),
                    startTime: $("#starttime").val() + ":00",
                    endDate: $("#todate").val(),
            ...
            if (response.messageCode == 0000 || response.messageCode == null) {
                ...
                if (NumPage == 0) {
                    if (status == 1) {
                        ...
                        htmlHead += "<th style='width: 8%;' set-lan='text:Amount'></th>";
                        htmlHead += "</tr></thead>";

                        htmlFoot += "<tfoot class='rgba-yellow-slight'>";
                        htmlFoot += "<tr class='total'>";
                        htmlFoot += "<td></td><td></td>";
                        ...
                        if (response.total > 0) {
                            for (var i = 0; i < response.data.detail.length; i++) {
                                ...
                                htmlData += "<td class='alignright'>" + amount + "</td>";
                                htmlData += "</tr>";
                            ...
                            var realizedAmount = parseFloat(response.data.totalPrize).toFixed(2);
                            ...
                            htmlData += '<tr><td colspan="4" class="aligncenter" set-lan="text:No data."></td></tr>';
                        ...
                        var n = arrNav.search("AA");
                        arrNav += navMenu;
                        if (n < 0) {
                            if (arrNav == "" || arrNav == null) {
                                navMenu = "<a class='linkNav AA' onclick='Check(`2`,``,`" + username + "`)'>" + username + "</a>";
                            }
                            else {
                                navMenu = " / <a class='linkNav AA' onclick='Check(`2`,``,`" + username + "`)'>" + username + "</a>";
                            }
                        ...
                        htmlHead += "<thead class='rgba-green-slight'><tr>";
                        htmlHead += "<th style='width: 2%;' set-lan='text:No'></th>";
                        htmlHead += "<th style='width: 15%; text-align: left; padding-left: 5px;' set-lan='text:Information'></th>";
                        htmlHead += "<th style='width: 7%; text-align: left; padding-left: 5px;' set-lan='text:Username'></th>";
                        htmlHead += "<th style='width: 6%;' set-lan='text:Level'></th>";
                        htmlHead += "<th style='width: 5%;' set-lan='text:Ranking No.'></th>";
                        htmlHead += "<th style='width: 5%;' set-lan='text:Score'></th>";
                        htmlHead += "<th style='width: 7%;' set-lan='text:Amount'></th>";
                        htmlHead += "<th style='width: 10%' set-lan='text:Remark'></th>";
                        htmlHead += "</tr></thead>";
                        ...
                        if (response.total != 0) {
                            for (var i = 0; i < response.data.detail.length; i++) {
                                ...
                                var date = new Date(obj.prizeStart);
                                var prizeStart;
                                if (obj.prizeStart == "" || obj.prizeStart == null) {
                                    prizeStart = "-";
                                } else {
                                    prizeStart = ("0" + date.getDate()).slice(-2) + "/" + ("0" + (date.getMonth() + 1)).slice(-2) + "/" + date.getFullYear() + " " + ("0" + date.getHours()).slice(-2) + ":" + ("0" + date.getMinutes()).slice(-2) + ":" + ("0" + date.getSeconds()).slice(-2);
                                }

                                var dateEnd = new Date(obj.prizeEnd);
                                var prizeEnd;
                                if (obj.prizeEnd == "" || obj.prizeEnd == null) {
                                    prizeEnd = "-";
                                } else {
                                    prizeEnd = ("0" + dateEnd.getDate()).slice(-2) + "/" + ("0" + (dateEnd.getMonth() + 1)).slice(-2) + "/" + dateEnd.getFullYear() + " " + ("0" + dateEnd.getHours()).slice(-2) + ":" + ("0" + dateEnd.getMinutes()).slice(-2) + ":" + ("0" + dateEnd.getSeconds()).slice(-2);
                                }

                                var dateAt = new Date(obj.prizeAt);
                                var prizeAt;
                                if (obj.prizeAt == "" || obj.prizeAt == null) {
                                    prizeAt = "-";
                                } else {
                                    prizeAt = ("0" + dateAt.getDate()).slice(-2) + "/" + ("0" + (dateAt.getMonth() + 1)).slice(-2) + "/" + dateAt.getFullYear() + " " + ("0" + dateAt.getHours()).slice(-2) + ":" + ("0" + dateAt.getMinutes()).slice(-2) + ":" + ("0" + dateAt.getSeconds()).slice(-2);
                                }
                                ...
                                htmlData += "<tr>";
                                htmlData += "<td class='aligncenter'>" + (((num - 1) * 100) + no) + "</td>";
                                htmlData += "<td><b set-lan='text:Prize Type'></b>: <span set-lan='text:" + prizeType + "'></span><br><b set-lan='text:Tournament start date'></b>: " + prizeStart + "<br><b set-lan='text:Tournament end date'></b>: " + prizeEnd + "<br><b set-lan='text:Transaction date'></b>: " + prizeAt + "</td>";
                                htmlData += "<td>" + obj.username + "</td>";
                                htmlData += "<td class='aligncenter' set-lan='text:" + Level + "'></td>";
                                htmlData += "<td class='alignright'>" + obj.rankNo + "</td>";
                                htmlData += "<td class='alignright'>" + obj.score + "</td>";
                                htmlData += "<td class='alignright'>" + amount + "</td>";
                                var nLineRemark = NewLine(obj.remark, 30);
