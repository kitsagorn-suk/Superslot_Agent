<%@ Page Title="" MasterPageFile="~/Site.Master" Language="C#" AutoEventWireup="true" CodeBehind="red_report.aspx.cs" Inherits="Pretty_Gaming.Menu_Report.red_report" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active txtMem" set-lan="text:Red Envelope Report"></li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-md-8">
            <h1 set-lan="text:Red Envelope Report"></h1>
        </div>
    </div>
    ...
    <div class="table-wrapper" style="margin-top: 10px; margin-bottom: 10px;">
        <table class="table table-border tableMaxWidth table-striped" id="tbData">
        </table>
    </div>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    ...
    <script type="text/javascript">      
        ...
        const postData = async (status, userID, username, num) => {
            ...
            var parameter = {};
            if (status == 1) {
                URL = `${apiURL}/v1/randomJackpot/report/stakeholder_payment`;
                parameter = {
                ...
                URL = `${apiURL}/v1/randomJackpot/report/open_history`;
                parameter = {
                    startDate: $("#startdate").val(),
                    startTime: $("#starttime").val() + ":00",
                    endDate: $("#todate").val(),
                ...
                if (NumPage == 0) {
                    if (status == 1) {
                        ...
                        if (response.total > 0) {
                            ...
                            var realizedAmount = parseFloat(response.data.totalAmount).toFixed(2);
                        ...
                        htmlHead += "<thead class='rgba-green-slight'><tr>";
                        htmlHead += "<th style='width: 1%;' set-lan='text:No'></th>";
                        htmlHead += "<th style='width: 5%; text-align: left; padding-left: 5px;' set-lan='text:Information'></th>";
                        htmlHead += "<th style='width: 5%; text-align: right; padding-right: 5px;' set-lan='text:Goal'></th>";
                        htmlHead += "<th style='width: 5%; text-align: right; padding-right: 5px;' set-lan='text:Tier'></th>";
                        htmlHead += "<th style='width: 5%;'><span set-lan='text:Reward'></span> (THB)</th>";
                        htmlHead += "<th style='width: 5%; text-align: right; padding-right: 5px;' set-lan='text:Total Reward'></th>";
                        htmlHead += "</tr></thead>";

                        htmlFoot += "<tfoot class='rgba-yellow-slight'>";
                        htmlFoot += "<tr class='total'>";
                        htmlFoot += "<td></td><td></td><td></td><td></td>";
                        htmlFoot += "<td set-lan='text:Total'></td>";
                        htmlFoot += "<td id='totalReward'></td>";
                        htmlFoot += "</tr></tfoot>";
                        ...
                        if (response.total != 0) {
                            for (var i = 0; i < response.data.details.length; i++) {
                                ...
                                var txtReward = "";
                                for (var k = 0; k < obj.rewardDetail.length; k++) {
                                    var SetID = k + 1;
                                    var Reward = parseFloat(obj.rewardDetail[k].rewardAmount).toFixed(2);
                                    Reward = Reward.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
                                    txtReward += "<b set-lan='text:Open " + SetID + " times'></b><span class='FloatR'>" + Reward + "</span><br>";
                                }

                                var totalReward = parseFloat(obj.totalReward).toFixed(2);
                                totalReward = totalReward.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");

                                htmlData += "<tr>";
                                htmlData += "<td class='aligncenter'>" + (((num - 1) * 100) + no) + "</td>";
                                htmlData += "<td><b set-lan='text:User'></b>: " + obj.username + "<br><b set-lan='text:Date'></b>: " + txtDate + "</td>"
                                htmlData += "<td class='alignright'>" + obj.goal + "</td>";
                                htmlData += "<td class='alignright'>" + obj.tier + "</td>";
                                htmlData += "<td>" + txtReward + "</td>";
                                htmlData += "<td class='alignright'>" + totalReward + "</td>";
                                htmlData += "</tr>";
                                no++;
                            }
                            htmlAll = htmlHead + htmlData + htmlFoot;
                            ...
                            var totalAmount = parseFloat(response.data.totalAmount).toFixed(2);
                            totalAmount = totalAmount.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
                            $("#totalReward").text(totalAmount.toString());

                            SetLan(localStorage.getItem("Language"));
                            $("#myModalLoad").modal('hide');
                        }
                        else {
                            htmlData += "<tr><td colspan='6' class='aligncenter' set-lan='text:No data.'></td></tr>";
                            htmlAll = htmlHead + htmlData + htmlFoot;
                            $("#tbData").append(htmlAll);
