<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="wl_report_support.aspx.cs" Inherits="Pretty_Gaming.Menu_Report.wl_report_support" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainScript" runat="server">
    ...
    <script type="text/javascript">     
        ...
        $(document).ready(function () {
            $("#menuReportSup , #menuReportSup > a").addClass("active");
            $("#menuReportSup > div").css("display", "block");
        ...
        function btnExport() {
            ...
            if ("ActiveXObject" in window) {
                ...
                var txtTemp = "";
                if (arrBet.length == 1) {
                    txtTemp = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{{=worksheet}}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table><tr><td colspan="10" style="font-weight: bold; text-align: center;">Report W/L Report : ' + arrBet + '</td></tr><tr><td colspan="10" style="font-weight: bold; text-align: center;">Start date : ' + startDate + '&emsp;To date : ' + toDate + '</td></tr><tr><td colspan="10" style="font-weight: bold; text-align: center;"</td></tr></table>{{for(var i=0; i<tables.length;i++){ }}<table>{{=tables[i]}}</table>{{ } }}</body></html>';
                }
                else {
                    txtTemp = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{{=worksheet}}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table><tr><td colspan="16" style="font-weight: bold; text-align: center;">Report W/L Report : ' + arrBet + '</td></tr><tr><td colspan="16" style="font-weight: bold; text-align: center;">Start date : ' + startDate + '&emsp;To date : ' + toDate + '</td></tr><tr><td colspan="16" style="font-weight: bold; text-align: center;"></td></tr></table>{{for(var i=0; i<tables.length;i++){ }}<table>{{=tables[i]}}</table>{{ } }}</body></html>';
                }
                        ...
                        if (response.total > 0) {
                            ...
                            for (var i = 0; i < response.data.detail.length; i++) {
                                ...
                                if (response.data.detail[i].position == "MEMBER") {
                                    ...
                                    htmlData += "<td style='width: 7.5%; left: 2%;'><p onclick='GetDataTable1(`" + response.data.detail[i].userID + "`," + num + ",`" + username + "`)' class='linkNav overflow ellipsis linkUser' title='" + response.data.detail[i].username + "'>" + response.data.detail[i].username + "</p></td>";
                                }
                            ...
                            htmlData += "<tr><td colspan='14' style='text-align: center;' set-lan='text:No data.'></td></tr>";
                            htmlAll = htmlHead + htmlData + htmlFoot;
                            $("#tbData").append(htmlAll);
                            $("#totalValid").text("-");
                            $("#totalWL").text("-");
